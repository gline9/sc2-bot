...
public class SupplyCapStrategy
{
    ...
    public SupplyCapStrategy(S2Agent agent, BuildingPlacementStrategy buildingPlacementStrategy, UnitPool unitPool)
    {
        this.agent = agent;
    ...
    public void queueSupplyDepotBuild()
    {
        ...
        Set<? extends SCV> scvs = unitPool.getUnitsOfType(SCV.class);

        SCV scv = scvs.iterator().next();

        Command<? super SCV> previousCommand = scv.getCommand();

        BuildStructureCommand buildSupplyDepotCommand = new BuildStructureCommand(agent, buildingPlacementStrategy, Abilities.BUILD_SUPPLY_DEPOT);
